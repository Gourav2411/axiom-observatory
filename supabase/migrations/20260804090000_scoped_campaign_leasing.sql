...
begin
  ...
  if exists (select 1 from unnest(p_job_types) as requested(job_type) where requested.job_type not in ('molecule_prep','admet','docking_prepare','docking_score','retrosynthesis_fragments','route_planning')) then
