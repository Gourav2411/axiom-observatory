...
function TranslationPanel({run,accessToken}) {
  ...
  const switchPhase=(next)=>{setPhase(next);const domain=candidate?.clinicalReadiness?.[next]?.domains?.[0]?.key??"";setForm(current=>({...current,domain}));};
  ...
  return <div className="live-translation">
    <header className="live-validation-header"><div><span className="live-kicker"><Flask/> Model-informed translation</span><h2>Phase I and Phase II simulation readiness</h2><p>Register and qualify the evidence required by clinical simulation workers. This workspace shows readiness and blockers; it never turns missing inputs into simulated patients or outcomes.</p></div><button onClick={load}><ArrowClockwise/>Refresh</button></header>
    {error&&<div className="live-error"><Warning/>{error}</div>}
    {!candidate?<Empty title="No campaign candidate available" detail="Create a campaign and ingest a candidate before assessing clinical translation."/>:<>
      ...
      <aside className="live-engine-column"><label>Simulation workers</label>{readiness?.engines?.map(engine=><div className={engine.configured?"is-ready":""} key={engine.id}><span>{engine.configured?<CheckCircle/>:<ShieldWarning/>}</span><strong>{engine.label}</strong><small>{engine.configured?"Endpoint configured · validation still required":"Not configured"}</small></div>)}<button disabled><Play/>{readiness?.executionStatus==="ready_for_engine_execution"?"Execution adapter ready":"Run simulation · blocked"}</button><p>{readiness?.executionStatus==="blocked_missing_qualified_inputs"?"Complete and qualify every evidence domain before configuring a simulation.":readiness?.executionStatus==="blocked_missing_simulation_engines"?"Evidence is ready; install and validate the required open-source simulation workers.":"Execution requires a reviewed scenario and authorized worker invocation."}</p></aside></section>
    </>}
  </div>;
}
