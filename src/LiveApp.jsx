...
function defaultClinicalScenario(phase) {
  const scenario = {
    seed: 20260804,
    pk: {
      doseMg: 100, doseCount: 7, doseIntervalHours: 24, durationHours: 168,
      ...
      betweenSubjectCv: 0.3, residualCv: 0.12, cohortSize: 250,
    },
  };
  if (phase === "phase2") scenario.pd = {
    emax: 8, ec50NgPerMl: 500, placeboChange: -1, endpointSd: 7,
    treatmentN: 120, controlN: 120, dropoutRate: 0.15, trialReplicates: 1000,
  };
  return scenario;
}
...
function Capabilities({run}) {
  ...
  if(local){
    ...
    values.admet={label:"ADMET and toxicity",status:local.admet?.available?"available_local":local.admet?.batchAvailable?"available_async":"unavailable",available:Boolean(local.admet?.available||local.admet?.batchAvailable),provider:local.admet?.batchProvider??local.admet?.provider,model:local.admet?.version,detail:local.admet?.available?"ADMET-AI prediction is available locally. Outputs are computational predictions, not measured safety data.":local.admet?.reason??"ADMET-AI is unavailable."};
    values.docking={label:"Docking",status:local.docking?.available?"available_local":local.docking?.batchAvailable?"available_async":local.docking?.preparationAvailable?"preparation_only":"unavailable",available:Boolean(local.docking?.available||local.docking?.batchAvailable),provider:local.docking?.batchProvider??local.docking?.provider,model:local.docking?.version,detail:local.docking?.available?"Vina scoring and ligand preparation are available locally.":local.docking?.reason??(local.docking?.preparationAvailable?"Meeko ligand preparation is available; Vina pose scoring is unavailable until a compatible binary and prepared receptor are configured.":"Docking is unavailable.")};
...
function ValidationPlan({run, accessToken}) {
  ...
  useEffect(()=>{
    if(!admetQueue?.candidateId||!["queued","leased","running"].includes(admetQueue.status))return undefined;
    let active=true;
    const refresh=async()=>{
      try{
  ...
  const toolCards=[
    ...
    ["ADMET & toxicity",ChartLineUp,capabilities.admet?.available?"READY":capabilities.admet?.batchAvailable?"BATCH QUEUE":"BLOCKED",capabilities.admet?.batchProvider??capabilities.admet?.provider,capabilities.admet?.version,capabilities.admet?.reason??"Run the complete physicochemical, ADME and toxicity panel using model inference."],
    ["Docking",Cube,capabilities.docking?.available?"SCORING READY":capabilities.docking?.batchAvailable?"BATCH QUEUE":capabilities.docking?.preparationAvailable?"PREP ONLY":"BLOCKED",capabilities.docking?.batchProvider??capabilities.docking?.provider,capabilities.docking?.version,capabilities.docking?.batchAvailable?"Vina scoring runs asynchronously after candidates are queued in a campaign.":"Prepare a Meeko ligand and reproducible Vina manifest; scoring waits for a compatible Vina engine."],
  ...
  return <div className="live-validation">
    <header className="live-validation-header">
      <div><span className="live-kicker"><Flask/> Computational validation</span><h2>Open-source validation workbench</h2><p>Run lightweight chemistry interactively and submit ADMET or Vina computation through durable campaign batches. Every result carries its model, version, method, artifacts and scientific boundary.</p></div>
    ...
    <section className="live-chemistry-workbench">
      ...
      {tool==="ADMET & toxicity"&&<div className="live-chem-tool"><section className="live-chem-input"><label>Prediction input</label><code className="live-input-code">{prepared?.canonicalSmiles??smiles}</code><button className="live-primary-action" onClick={capabilities.admet?.available?predict:queueAdmet} disabled={(!capabilities.admet?.available&&!capabilities.admet?.batchAvailable)||!smiles.trim()||Boolean(running)||["queued","leased","running"].includes(admetQueue?.status)}><Play/>{running==="admet"?"Running endpoint panel…":running==="admet-queue"?"Queuing GitHub run…":["queued","leased","running"].includes(admetQueue?.status)?`ADMET ${humanize(admetQueue.status)}…`:capabilities.admet?.batchAvailable?"Run ADMET asynchronously":"Run ADMET-AI"}</button><p>{capabilities.admet?.batchAvailable?"This click persists the candidate in Supabase, queues ADMET-AI, dispatches GitHub Actions, and returns the result here when complete.":"First inference loads the model ensembles and can take several seconds. Outputs are predictions, never measured results."}</p>{admetQueue&&<div className={`live-async-status is-${statusTone(admetQueue.status)}`}><ArrowClockwise/><span><strong>{humanize(admetQueue.status)}</strong><small>{admetQueue.compute?.status==="dispatched"?"GitHub Actions dispatched immediately":admetQueue.compute?.reason??"Durable Supabase job"}{admetQueue.pollError?` · refresh warning: ${admetQueue.pollError}`:""}</small></span></div>}</section><section className="live-chem-output">{admet?<><div className="live-admet-header"><span><strong>{admet.endpoints?.length} endpoints</strong><small>{admet.provenance?.provider} {admet.provenance?.version} · {admet.provenance?.durationMs} ms</small></span><b>COMPUTATIONAL PREDICTION</b></div><div className="live-admet-grid">{admet.highlights?.map(item=><article key={item.id}><span>{item.category}</span><strong>{item.name}</strong><b>{Number(item.value).toFixed(3)}{item.units&&item.units!=="-"?` ${item.units}`:""}</b><small>{item.interpretation}</small></article>)}</div><details className="live-endpoint-details"><summary>View all {admet.endpoints?.length} endpoints</summary><div>{admet.endpoints?.map(item=><p key={item.id}><span>{item.name}</span><code>{Number(item.value).toFixed(4)} {item.units&&item.units!=="-"?item.units:""}</code></p>)}</div></details><p className="live-boundary">{admet.boundary}</p></>:<Empty title="No ADMET inference" detail={capabilities.admet?.batchAvailable?"Click Run ADMET asynchronously. The durable result will appear here after the GitHub Actions worker completes.":"Run ADMET-AI to calculate the complete local prediction panel."}/>}</section></div>}
      {tool==="Docking"&&<div className="live-chem-tool"><section className="live-chem-input"><label>Receptor and search box</label><input placeholder="PDB ID or prepared receptor ID" value={dockInput.receptor_id} onChange={event=>setDockInput(current=>({...current,receptor_id:event.target.value}))}/><label>Prepared rigid receptor · PDBQT</label><input type="file" accept=".pdbqt,chemical/x-pdbqt,text/plain" onChange={uploadReceptor} disabled={running}/>{receptor&&<small>Registered {receptor.receptorPath} · SHA-256 {receptor.sha256.slice(0,12)}…</small>}<div className="live-vector-inputs"><span>Center</span>{["x","y","z"].map(axis=><input key={`c${axis}`} aria-label={`Center ${axis}`} type="number" value={dockInput.center[axis]} onChange={event=>setDockInput(current=>({...current,center:{...current.center,[axis]:Number(event.target.value)}}))}/>)}</div><div className="live-vector-inputs"><span>Size Å</span>{["x","y","z"].map(axis=><input key={`s${axis}`} aria-label={`Size ${axis}`} type="number" value={dockInput.size[axis]} onChange={event=>setDockInput(current=>({...current,size:{...current.size,[axis]:Number(event.target.value)}}))}/>)}</div><label>Known-ligand control SMILES · recommended</label><input value={dockInput.control_smiles} onChange={event=>setDockInput(current=>({...current,control_smiles:event.target.value}))}/><button className="live-primary-action" onClick={capabilities.docking?.available&&dockInput.receptor_path?runDocking:prepareDocking} disabled={!capabilities.docking?.preparationAvailable||!dockInput.receptor_id.trim()||!smiles.trim()||running}><Cube/>{running==="docking-run"?"Running Vina replicates…":running==="docking"?"Preparing ligand…":capabilities.docking?.available&&dockInput.receptor_path?"Run Vina docking":"Prepare docking package"}</button><p>{capabilities.docking?.available?"Runs three deterministic Vina seed replicates and an optional same-box known-ligand control.":"Creates a Meeko PDBQT ligand and deterministic Vina configuration; scoring requires the private compute service."}</p></section><section className="live-chem-output">{docking?<><div className="live-docking-state"><CheckCircle/><span><strong>{docking.bestAffinity!=null?`${Number(docking.bestAffinity).toFixed(2)} kcal/mol best score`:"Docking package prepared"}</strong><small>{docking.status.replaceAll("_"," ")}</small></span></div><dl className="live-config-list">{docking.bestAffinity!=null&&<div><dt>Replicate stability</dt><dd>{docking.stability?.status} · σ {Number(docking.affinityStandardDeviation).toFixed(3)}</dd></div>}{Object.entries(docking.config??{}).map(([name,value])=><div key={name}><dt>{humanize(name)}</dt><dd>{typeof value==="object"?JSON.stringify(value):value}</dd></div>)}</dl><p className="live-boundary">{docking.boundary}</p></>:<Empty title="No docking result" detail="Upload a prepared receptor, define its pocket, and run Vina with a known-ligand control where available."/>}</section></div>}
...
function CampaignPanel({run, accessToken}) {
  ...
  const [campaignForm,setCampaignForm]=useState({name:`${run.target?.label??"Target"} lead campaign`,objective:"Prioritize candidate structures for scientific review",receptorId:run.target?.structure?.pdbId??"",receptorPath:"",receptorObject:null,receptorFileName:"",center:{x:0,y:0,z:0},size:{x:22,y:22,z:22},exhaustiveness:8,controlSmiles:""});
  ...
  const create=()=>perform("create",()=>api.createCampaign(run.id,{name:campaignForm.name,objective:campaignForm.objective,settings:{receptorId:campaignForm.receptorId,receptorPath:campaignForm.receptorPath,receptorObject:campaignForm.receptorObject,center:campaignForm.center,size:campaignForm.size,exhaustiveness:Number(campaignForm.exhaustiveness),seed:20260803,dockingReplicates:3,controlSmiles:campaignForm.controlSmiles,maxTransforms:6,routeTimeLimitSeconds:120}},accessToken));
  ...
  const assayDraft=(candidateId)=>assays[candidateId]??{assayType:"binding",endpoint:"IC50",value:"",unit:"nM",qualifier:"=",controlType:"reference",qcStatus:"pending",sourceReference:""};
  const ingestAssay=(candidate)=>{const draft=assayDraft(candidate.id);return perform(`assay-${candidate.id}`,()=>api.ingestAssay(candidate.id,{assayType:draft.assayType,endpoint:draft.endpoint,value:Number(draft.value),unit:draft.unit,qualifier:draft.qualifier,controlType:draft.controlType,qcStatus:draft.qcStatus,sourceType:"experimental",protocol:{},provenance:{sourceReference:draft.sourceReference}},accessToken));};
  const vector=(field,axis,value)=>setCampaignForm(current=>({...current,[field]:{...current[field],[axis]:Number(value)}}));
  const loadCampaignReceptor=async(event)=>{const file=event.target.files?.[0];if(!file)return;if(file.size>25_000_000){setError("Prepared receptor PDBQT must be smaller than 25 MB.");return;}const receptorId=campaignForm.receptorId||file.name.replace(/\.pdbqt$/i,"");setBusy("receptor-upload");setError("");try{const stored=await api.uploadReceptor(run.id,{receptor_id:receptorId,pdbqt:await file.text()},accessToken);setCampaignForm(current=>({...current,receptorId:stored.receptorId,receptorObject:{bucketId:stored.bucketId,objectPath:stored.objectPath,sha256:stored.sha256},receptorFileName:file.name,receptorPath:""}));}catch(uploadError){setError(uploadError.message);}finally{setBusy("");}};
  ...
  return <div className="live-campaign">
    ...
    <>
      ...
      <div className="live-campaign-gate"><ShieldWarning/><span><strong>Real execution, capability-gated</strong><small>RDKit and BRICS run on the web service. ADMET-AI and Vina run in asynchronous GitHub Actions batches; Vina ranking requires a versioned receptor, three stable seed replicates and a same-pocket control. Missing capabilities finish as blocked, never simulated.</small></span></div>
      {!campaign.candidates?.length?<Empty title="No candidates ingested" detail="Add a named SMILES structure to begin the campaign."/>:<section className="live-candidate-list">{campaign.candidates.map((candidate,index)=>{
        ...
        return <article className="live-candidate" key={candidate.id}><header><i>{String(index+1).padStart(2,"0")}</i><div><strong>{candidate.name}</strong><code>{candidate.canonical_smiles??candidate.input_smiles}</code></div><span><b>{candidate.rank_score==null?"—":Number(candidate.rank_score).toFixed(1)}</b><small>priority / 100</small></span><em className={`is-${statusTone(candidate.status)}`}>{humanize(candidate.status)}</em></header>
          ...
          <div className="live-candidate-body"><section><label>Model and method boundaries</label>{candidate.evaluations?.length?<div className="live-evaluations">{candidate.evaluations.map(item=><details key={item.id}><summary><span>{humanize(item.evaluation_type)}</span><b className={`is-${statusTone(item.status)}`}>{humanize(item.status)}</b></summary><p>{item.boundary}</p><dl><div><dt>Applicability</dt><dd>{humanize(item.applicability?.status??"not assessed")}</dd></div><div><dt>Ranking method</dt><dd>{item.score_component?.method??"Excluded from ranking"}</dd></div></dl></details>)}</div>:<p className="live-muted">No worker results yet.</p>}<button className="live-secondary-action" onClick={()=>queue(candidate)} disabled={busy||["queued","evaluating"].includes(candidate.status)}><Play/>{busy===`queue-${candidate.id}`?"Queueing…":candidate.jobs?.length?"Requeue blocked or failed jobs":"Queue six chemistry jobs"}</button>
          <div className="live-assay-ingest"><label>Measured assay result</label><div className="live-assay-grid"><input aria-label="Assay type" placeholder="Assay type" value={assayDraft(candidate.id).assayType} onChange={event=>setAssays(current=>({...current,[candidate.id]:{...assayDraft(candidate.id),assayType:event.target.value}}))}/><input aria-label="Endpoint" placeholder="Endpoint" value={assayDraft(candidate.id).endpoint} onChange={event=>setAssays(current=>({...current,[candidate.id]:{...assayDraft(candidate.id),endpoint:event.target.value}}))}/><input aria-label="Measured value" type="number" placeholder="Value" value={assayDraft(candidate.id).value} onChange={event=>setAssays(current=>({...current,[candidate.id]:{...assayDraft(candidate.id),value:event.target.value}}))}/><input aria-label="Unit" placeholder="Unit" value={assayDraft(candidate.id).unit} onChange={event=>setAssays(current=>({...current,[candidate.id]:{...assayDraft(candidate.id),unit:event.target.value}}))}/></div><input aria-label="Assay source reference" placeholder="ELN record, DOI, report or plate ID" value={assayDraft(candidate.id).sourceReference} onChange={event=>setAssays(current=>({...current,[candidate.id]:{...assayDraft(candidate.id),sourceReference:event.target.value}}))}/><button className="live-secondary-action" onClick={()=>ingestAssay(candidate)} disabled={busy||!assayDraft(candidate.id).sourceReference.trim()||!Number.isFinite(Number(assayDraft(candidate.id).value))||assayDraft(candidate.id).value===""}><Database/>{busy===`assay-${candidate.id}`?"Recording…":"Record provenance-linked assay"}</button>{candidate.assays?.map(item=><div className="live-assay-record" key={item.id}><strong>{item.assay_type} · {item.endpoint}</strong><code>{item.qualifier} {item.value} {item.unit}</code><small>{humanize(item.qc_status)} · {item.provenance?.sourceReference}</small></div>)}</div>
          <div className="live-readiness"><label>Clinical translation readiness</label>{[candidate.clinicalReadiness?.phase1,candidate.clinicalReadiness?.phase2].map((phase,phaseIndex)=><div key={phaseIndex}><strong>Phase {phaseIndex+1}</strong><em className={`is-${phase?.status?.startsWith("ready")?"success":"warning"}`}>{humanize(phase?.status??"not assessed")}</em><small>{phase?.domains?.filter(item=>item.status!=="evidence_supplied").length??0} qualified input domains missing</small></div>)}<p>{candidate.clinicalReadiness?.boundary}</p></div></section>
...
function TranslationPanel({run,accessToken}) {
  const [campaigns,setCampaigns]=useState([]); const [candidateId,setCandidateId]=useState(""); const [phase,setPhase]=useState("phase1");
  const [loading,setLoading]=useState(true); const [busy,setBusy]=useState(""); const [error,setError]=useState("");
  const [form,setForm]=useState({domain:"identity",inputKind:"document",sourceReference:"",notes:""}); const [reviewDrafts,setReviewDrafts]=useState({});
  const [mode,setMode]=useState("research_scenario"); const [scenario,setScenario]=useState(()=>defaultClinicalScenario("phase1"));
  const load=async()=>{setError("");try{const value=await api.listCampaigns(run.id,accessToken);setCampaigns(value.items??[]);const candidates=(value.items??[]).flatMap(item=>item.candidates??[]);setCandidateId(current=>candidates.some(item=>item.id===current)?current:(candidates[0]?.id??""));}catch(e){setError(e.message);}finally{setLoading(false);}};
  useEffect(()=>{load();},[run.id,accessToken]);
  const candidates=campaigns.flatMap(campaign=>(campaign.candidates??[]).map(candidate=>({...candidate,campaignName:campaign.name})));
  const candidate=candidates.find(item=>item.id===candidateId)??candidates[0]; const readiness=candidate?.clinicalReadiness?.[phase];
  const switchPhase=(next)=>{setPhase(next);setScenario(defaultClinicalScenario(next));const domain=candidate?.clinicalReadiness?.[next]?.domains?.[0]?.key??"";setForm(current=>({...current,domain}));};
  useEffect(()=>{if(readiness?.domains?.length&&!readiness.domains.some(item=>item.key===form.domain))setForm(current=>({...current,domain:readiness.domains[0].key}));},[candidateId,phase,readiness?.domains?.length]);
  const perform=async(key,action)=>{setBusy(key);setError("");try{await action();await load();}catch(e){setError(e.message);}finally{setBusy("");}};
  const register=()=>perform("register",()=>api.registerTranslationInput(candidate.id,{phase,domain:form.domain,inputKind:form.inputKind,sourceReference:form.sourceReference,payload:{notes:form.notes}},accessToken));
  const review=(input,decision)=>{const rationale=reviewDrafts[input.id]??"";return perform(`review-${input.id}`,()=>api.reviewTranslationInput(input.id,{decision,rationale},accessToken));};
  const phaseInputs=(candidate?.translationInputs??[]).filter(input=>input.phase===phase); const missing=readiness?.domains?.filter(item=>item.status!=="evidence_supplied")??[];
  const simulations=(candidate?.simulations??[]).filter(item=>item.phase===phase); const latest=simulations[0];
  const qualifiedReady=missing.length===0; const canRun=mode==="research_scenario"||qualifiedReady;
  const setPk=(key,value)=>setScenario(current=>({...current,pk:{...current.pk,[key]:Number(value)}}));
  const setPd=(key,value)=>setScenario(current=>({...current,pd:{...current.pd,[key]:Number(value)}}));
  const queueSimulation=()=>perform("simulation",()=>api.queueClinicalSimulation(candidate.id,{phase,mode,scenario},accessToken));
  useEffect(()=>{if(!["queued","leased","running"].includes(latest?.status))return;const timer=setInterval(load,5000);return()=>clearInterval(timer);},[latest?.id,latest?.status]);
  const exposure=latest?.result?.result?.exposure; const operating=latest?.result?.result?.operatingCharacteristics;
  const pkFields=[
    ["doseMg","Dose","mg"],["doseCount","Doses","count"],["doseIntervalHours","Interval","h"],["durationHours","Duration","h"],
    ["bioavailability","Bioavailability","fraction"],["absorptionRatePerHour","Absorption ka","1/h"],["clearanceLPerHour","Clearance","L/h"],
    ["centralVolumeL","Central volume","L"],["peripheralVolumeL","Peripheral volume","L"],["intercompartmentalClearanceLPerHour","Intercompartmental Q","L/h"],
    ["betweenSubjectCv","Between-subject CV","fraction"],["residualCv","Residual CV","fraction"],["cohortSize","Virtual cohort","subjects"],
  ];
  const pdFields=[["emax","Maximum effect","endpoint units"],["ec50NgPerMl","EC50","ng/mL"],["placeboChange","Placebo change","endpoint units"],["endpointSd","Endpoint SD","units"],["treatmentN","Treatment arm","subjects"],["controlN","Control arm","subjects"],["dropoutRate","Dropout","fraction"],["trialReplicates","Trial replicates","runs"]];
  if(loading)return <div className="live-campaign-loading"><ArrowClockwise/><span>Loading translation evidence…</span></div>;
  return <div className="live-translation">
    <header className="live-validation-header"><div><span className="live-kicker"><Flask/> Model-informed translation</span><h2>Clinical modeling laboratory</h2><p>Execute transparent population PK and PK/PD research scenarios, inspect uncertainty, and preserve every input and result as a reproducible campaign artifact.</p></div><button onClick={load}><ArrowClockwise/>Refresh</button></header>
    {error&&<div className="live-error"><Warning/>{error}</div>}
    {!candidate?<Empty title="No campaign candidate available" detail="Create a campaign and ingest a candidate before assessing clinical translation."/>:<>
      <section className="live-translation-command"><div><label>Candidate</label><select value={candidate.id} onChange={event=>setCandidateId(event.target.value)}>{candidates.map(item=><option value={item.id} key={item.id}>{item.name} · {item.campaignName}</option>)}</select><code>{candidate.canonical_smiles??candidate.input_smiles}</code></div><div className="live-phase-switch" role="tablist" aria-label="Clinical phase">{[["phase1","Phase I"],["phase2","Phase II"]].map(([value,label])=><button type="button" role="tab" aria-selected={phase===value} className={phase===value?"is-active":""} onClick={()=>switchPhase(value)} key={value}><span>{label}</span><small>{candidate.clinicalReadiness?.[value]?.domains?.filter(item=>item.status!=="evidence_supplied").length??0} inputs missing</small></button>)}</div></section>
      <section className="live-model-mode"><button className={mode==="research_scenario"?"is-active":""} onClick={()=>setMode("research_scenario")}><Flask/><span><strong>Research POC</strong><small>Runs now from declared assumptions · not clinical evidence</small></span></button><button className={mode==="evidence_qualified"?"is-active":""} onClick={()=>setMode("evidence_qualified")}><ShieldWarning/><span><strong>Evidence-qualified</strong><small>{qualifiedReady?"All prerequisite domains qualified":"Locked until every prerequisite is qualified"}</small></span></button></section>
      <div className="live-translation-boundary"><ShieldWarning/><span><strong>{latest?.status==="succeeded"?"Model projection completed":latest&&["queued","leased","running"].includes(latest.status)?`Simulation ${humanize(latest.status)}`:canRun?"Scenario ready to queue":"Evidence-qualified execution is blocked"}</strong><small>Outputs are model-generated projections from declared assumptions. They are not patient data, dose advice, evidence of safety or efficacy, or a clinical trial.</small></span></div>
      <section className="live-simulation-lab"><div className="live-scenario-panel"><header><div><label>Scenario configuration</label><strong>{phase==="phase1"?"Two-compartment population PK":"Exposure-response trial operating characteristics"}</strong></div><button onClick={()=>setScenario(defaultClinicalScenario(phase))}><ArrowClockwise/>Reset assumptions</button></header><div className="live-parameter-grid">{pkFields.map(([key,label,unit])=><label key={key}><span>{label}<small>{unit}</small></span><input type="number" step="any" value={scenario.pk[key]} onChange={event=>setPk(key,event.target.value)}/></label>)}</div>{phase==="phase2"&&<><div className="live-subsection-label">PK/PD and parallel-arm design</div><div className="live-parameter-grid">{pdFields.map(([key,label,unit])=><label key={key}><span>{label}<small>{unit}</small></span><input type="number" step="any" value={scenario.pd[key]} onChange={event=>setPd(key,event.target.value)}/></label>)}</div></>}<label className="live-seed-input"><span>Deterministic seed</span><input type="number" value={scenario.seed} onChange={event=>setScenario(current=>({...current,seed:Number(event.target.value)}))}/></label></div>
      <aside className="live-simulation-console"><label>Asynchronous worker</label><div className="live-engine-ready"><CheckCircle/><span><strong>Axiom Open PK/PD · RK4 v1</strong><small>Python standard library · deterministic · artifact-backed</small></span></div><button className="live-primary-action" disabled={busy||!canRun||["queued","leased","running"].includes(latest?.status)} onClick={queueSimulation}><Play/>{busy==="simulation"?"Persisting scenario…":["queued","leased","running"].includes(latest?.status)?`Simulation ${humanize(latest.status)}…`:`Run ${phase==="phase1"?"population PK":"PK/PD trial model"}`}</button><p>{mode==="research_scenario"?"Research mode is intentionally assumption-driven so the computational POC is usable before proprietary experimental data exist.":qualifiedReady?"All current phase prerequisites are qualified; the evidence snapshot will remain linked to this candidate.":`${missing.length} qualified evidence domains remain.`}</p>{simulations.length>0&&<div className="live-simulation-history"><label>Recent runs</label>{simulations.slice(0,4).map(item=><div key={item.id}><i className={`is-${statusTone(item.status)}`}/><span><strong>{humanize(item.status)}</strong><small>{humanize(item.mode)} · {new Date(item.created_at).toLocaleString()}</small></span><code>{item.scenario_sha256?.slice(0,8)}</code></div>)}</div>}</aside></section>
      {latest?.status==="succeeded"&&<section className="live-simulation-results"><header><div><label>Reproducible model output</label><h3>{phase==="phase1"?"Population exposure distribution":"Phase II operating characteristics"}</h3></div><span><b>MODEL PROJECTION</b><code>{latest.result?.model?.id}</code></span></header>{phase==="phase1"&&exposure&&<div className="live-result-metrics">{[["AUC",exposure.aucMgHourPerL,"mg·h/L"],["Cmax",exposure.cmaxNgPerMl,"ng/mL"],["Trough",exposure.troughNgPerMl,"ng/mL"],["Half-life",exposure.halfLifeHours,"h"]].map(([label,value,unit])=><article key={label}><span>{label}</span><strong>{value?.p50??"—"}</strong><small>P05 {value?.p05??"—"} · P95 {value?.p95??"—"} {unit}</small></article>)}</div>}{phase==="phase2"&&operating&&<div className="live-result-metrics">{[["Model-based success probability",operating.modelBasedProbabilityTwoSidedPBelow0_05,"two-sided p < 0.05"],["Median treatment difference",operating.treatmentDifference?.p50,"endpoint units"],["P05 difference",operating.treatmentDifference?.p05,"endpoint units"],["P95 difference",operating.treatmentDifference?.p95,"endpoint units"]].map(([label,value,unit])=><article key={label}><span>{label}</span><strong>{value??"—"}</strong><small>{unit}</small></article>)}</div>}<div className="live-result-provenance"><code>Input {latest.result?.inputSha256?.slice(0,16)}…</code><code>Seed {latest.result?.seed}</code><code>{latest.result?.provenance?.python?`Python ${latest.result.provenance.python}`:"Worker version retained"}</code></div><p>{latest.boundary}</p></section>}
      <details className="live-evidence-readiness"><summary><span><ShieldWarning/><strong>Scientific evidence gate</strong><small>{missing.length?`${missing.length} of ${readiness?.domains?.length??0} domains missing for evidence-qualified mode`:"All current phase domains qualified"}</small></span></summary><section className="live-translation-grid"><div className="live-domain-column"><header><label>Evidence gate</label><strong>{missing.length?`${missing.length} of ${readiness?.domains?.length??0} domains missing`:"All evidence domains qualified"}</strong></header><div className="live-domain-list">{readiness?.domains?.map((item,index)=><article className={item.status==="evidence_supplied"?"is-complete":""} key={item.key}><i>{item.status==="evidence_supplied"?<CheckCircle/>:String(index+1).padStart(2,"0")}</i><span><strong>{item.label}</strong><small>{item.status==="evidence_supplied"?"Qualified evidence registered":"Qualified evidence required"}</small>{item.sourceReferences?.map(source=><code key={source}>{source}</code>)}</span></article>)}</div></div>
      <div className="live-translation-actions"><section><label>Register source evidence</label><select value={form.domain} onChange={event=>setForm(current=>({...current,domain:event.target.value}))}>{readiness?.domains?.map(item=><option value={item.key} key={item.key}>{item.label}</option>)}</select><select value={form.inputKind} onChange={event=>setForm(current=>({...current,inputKind:event.target.value}))}><option value="document">Document or report</option><option value="measurement">Measured dataset</option><option value="model">Qualified model</option><option value="observation">Clinical observation</option></select><input placeholder="ELN record, DOI, report, dataset or protocol ID" value={form.sourceReference} onChange={event=>setForm(current=>({...current,sourceReference:event.target.value}))}/><textarea placeholder="Describe the population, species, matrix, assumptions and limitations…" value={form.notes} onChange={event=>setForm(current=>({...current,notes:event.target.value}))}/><button className="live-primary-action" disabled={busy||form.sourceReference.trim().length<2} onClick={register}><Database/>{busy==="register"?"Registering…":"Register for scientific review"}</button></section>
      <section><label>Scientific qualification queue</label>{!phaseInputs.length?<p className="live-muted">No evidence inputs registered for this phase.</p>:phaseInputs.map(input=><article className="live-translation-input" key={input.id}><header><strong>{humanize(input.domain)}</strong><em className={`is-${statusTone(input.review_status)}`}>{humanize(input.review_status)}</em></header><code>{input.source_reference}</code><small>{humanize(input.input_kind)} · {input.payload?.notes||"No notes supplied"}</small>{input.review_status==="pending"&&<><textarea placeholder="Qualification or rejection rationale…" value={reviewDrafts[input.id]??""} onChange={event=>setReviewDrafts(current=>({...current,[input.id]:event.target.value}))}/><div><button disabled={busy||(reviewDrafts[input.id]??"").trim().length<3} onClick={()=>review(input,"qualified")}><CheckCircle/>Qualify</button><button disabled={busy||(reviewDrafts[input.id]??"").trim().length<3} onClick={()=>review(input,"rejected")}><X/>Reject</button></div></>}</article>)}</section></div>
      <aside className="live-engine-column"><label>External validation path</label>{readiness?.engines?.map(engine=><div className={engine.configured?"is-ready":""} key={engine.id}><span>{engine.configured?<CheckCircle/>:<ShieldWarning/>}</span><strong>{engine.label}</strong><small>{engine.configured?"Endpoint configured · independent validation still required":"Optional external adapter not configured"}</small></div>)}<p>The built-in open model makes the POC executable. PK-Sim/MoBi, nlmixr2/rxode2 and regulatory validation remain explicit future qualification layers.</p></aside></section></details>
    </>}
  </div>;
}
...
function RunView({run,onNewRun,user,onSignOut,accessToken}) {
  ...
  const panel=tab==="Research"?<Research run={run} accessToken={accessToken}/>:tab==="Campaign"?<CampaignPanel run={run} accessToken={accessToken}/>:tab==="Translation"?<TranslationPanel run={run} accessToken={accessToken}/>:tab==="Index"?<RagIndex run={run}/>:tab==="Validation"?<ValidationPlan run={run} accessToken={accessToken}/>:tab==="Evidence"?<EvidenceTable run={run}/>:tab==="Literature"?<Literature run={run}/>:tab==="Provenance"?<Provenance run={run}/>:<Capabilities run={run}/>;
